...
import { Users, Quote, MapPin, ArrowRight } from 'lucide-react';

export default function ArtisansPage() {
  const [list, setList] = useState([]);

  useEffect(() => {
    fetch('/api/artisans').then((r) => r.json()).then((d) => setList(d.artisans || []));
  }, []);
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-museum text-amber-50">
        <div className="absolute inset-0">
          <img src="https://images.pexels.com/photos/17777833/pexels-photo-17777833.jpeg" alt="" className="w-full h-full object-cover opacity-30" />
          <div className="absolute inset-0 bg-gradient-to-b from-stone-950/80 via-stone-950/60 to-stone-950/95" />
        </div>
        <div className="relative max-w-6xl mx-auto px-6 pt-20 pb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-100/10 border border-amber-200/20 text-xs uppercase tracking-[0.25em] text-amber-200"><Users className="w-3.5 h-3.5" /> Hands behind the heritage</div>
          <h1 className="font-serif text-5xl md:text-7xl mt-5 leading-[1] text-shadow-museum max-w-4xl">Meet the artisans</h1>
          <p className="mt-5 text-lg max-w-2xl text-amber-100/85 leading-relaxed">Master weavers, mask-carvers, potters and painters whose hands keep these crafts alive. Profiles below are composite tributes to the living lineages of each tradition.</p>
        </div>
      </section>
